import React, { Component } from 'react';
import 'bootstrap/dist/css/bootstrap.min.css';
import 'bootstrap-icons/font/bootstrap-icons.css';

class Header extends Component {
  render() {
    return (
      <nav className="navbar navbar-expand-lg" style={{ backgroundColor: '#101010' }}>
        <div className="container-fluid">
          <div className="navbar-brand">
            <img src="data:image/png;base64," alt="netflix" style={{ width: '100%', height: '100%' }} />
          </div>
          <button
            className="navbar-toggler"
            type="button"
            data-bs-toggle="collapse"
            data-bs-target="#navbarSupportedContent"
            aria-controls="navbarSupportedContent"
            aria-expanded="false"
            aria-label="Toggle navigation"
          >
            <span className="navbar-toggler-icon"></span>
          </button>
          <div className="collapse navbar-collapse" id="navbarSupportedContent">
            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              <li className="nav-item">
                <button className="nav-link active fw-bold" style={{ background: 'none', border: 'none', color: '#f5f5f1' }}>Home</button>
              </li>
              <li className="nav-item">
                <button className="nav-link fw-bold" style={{ background: 'none', border: 'none', color: '#f5f5f1' }}>TV Shows</button>
              </li>
              <li className="nav-item">
                <button className="nav-link fw-bold" style={{ background: 'none', border: 'none', color: '#f5f5f1' }}>Movies</button>
              </li>
              <li className="nav-item">
                <button className="nav-link fw-bold" style={{ background: 'none', border: 'none', color: '#f5f5f1' }}>Recently Added</button>
              </li>
              <li className="nav-item">
                <button className="nav-link fw-bold" style={{ background: 'none', border: 'none', color: '#f5f5f1' }}>My List</button>
              </li>
            </ul>
            <div className="d-flex align-items-center">
              <i className="bi bi-search" style={{ marginLeft: '0.8em', marginRight: '0.8em', fontSize: '1.2em', color: '#f5f5f1' }}></i>
              <div id="kids" style={{ color: '#f5f5f1', fontSize: '0.8em' }} className="fw-bold">KIDS</div>
              <i className="bi bi-bell" style={{ marginLeft: '0.8em', marginRight: '0.8em', fontSize: '1.2em', color: '#f5f5f1' }}></i>
              <i className="bi bi-person-circle" style={{ marginLeft: '0.8em', marginRight: '0.8em', fontSize: '1.2em', color: '#f5f5f1' }}></i>
            </div>
          </div>
        </div>
      </nav>
    );
  }
}

export default Header;